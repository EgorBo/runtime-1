...
bool MyICJI::getReadonlyStaticFieldValue(CORINFO_FIELD_HANDLE field, uint8_t* buffer, int bufferSize, CorInfoObjectValueKind kind)
{
    jitInstance->mc->cr->AddCall("getReadonlyStaticFieldValue");
    return jitInstance->mc->repGetReadonlyStaticFieldValue(field, buffer, bufferSize, kind);
}
