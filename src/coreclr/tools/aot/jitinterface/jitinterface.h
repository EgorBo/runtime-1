...
struct JitInterfaceCallbacks
{
    ...
    bool (* getReadonlyStaticFieldValue)(void * thisHandle, CorInfoExceptionClass** ppException, CORINFO_FIELD_HANDLE field, uint8_t* buffer, int bufferSize, CorInfoObjectValueKind kind);
...
{
    CorInfoExceptionClass* pException = nullptr;
    bool temp = _callbacks->getReadonlyStaticFieldValue(_thisHandle, &pException, field, buffer, bufferSize, kind);
    if (pException != nullptr) throw pException;
    return temp;
}
