...
void Compiler::fgQuickPrescan(const BYTE* codeAddr, IL_OFFSET codeSize)
{
    assert(impInlineRoot()->opts.jitFlags->IsSet(JitFlags::JIT_FLAG_TIER0));
    ...
    DECODE_OPCODE:
        ...
        case CEE_SWITCH:
        {
            compInlineResult->Note(InlineObservation::CALLEE_HAS_BRANCH);
            assert(compInlineResult->IsFailure());
            return;
        }
    ...
    assert(!compInlineResult->IsDiscretionaryCandidate());
}
...
void Compiler::fgFindJumpTargets(const BYTE* codeAddr, IL_OFFSET codeSize, FixedBitVect* jumpTarget)
{
    ...
    const bool preciseScan            = makeInlineObservations && compInlineResult->GetPolicy()->RequiresPreciseScan();
    const bool tier0                  = impInlineRoot()->opts.jitFlags->IsSet(JitFlags::JIT_FLAG_TIER0);
    if (makeInlineObservations && tier0 && preciseScan)
    {
        fgQuickPrescan(codeAddr, codeSize);
        return;
    }
    ...
    if (makeInlineObservations || isInlining)
    {
        assert(!tier0);
    }
